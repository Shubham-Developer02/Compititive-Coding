...
// Range update ... i mean update at certain range with some fixed value  
void Range_update(int *tree,int ss,int se,int start_pos,int end_pos,int increament,int index)
{
    // Out of Bounds
    ...
    int mid = (ss + se)/2 ;
    Range_update(tree,ss,mid,start_pos,end_pos,increament,2*index) ;
    Range_update(tree,mid+1,se,start_pos,end_pos,increament,2*index+1) ;
    tree[index] = min( tree[2*index] , tree[2*index+1] ) ;
    return ;
